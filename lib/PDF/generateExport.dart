...
Future<Uint8List> generateReport(
    PdfPageFormat pageFormat, Hyetograph hyetograph, Units unit) async {
  List<double> dataTable = hyetograph.getData(unit);
  var duration = hyetograph.getDurations();
  var tableHeaders = <dynamic>[
    "Periodo de Retorno",
    ...duration.map((e) => "$e min")
  ];

  double maxData = dataTable.reduce((a, b) => a > b ? a : b) * 1.2;
  List<double> listInY = List<double>.generate(
      10, (index) => (maxData * (index / 10)).toPrecision(0));
